...
import dev.eatgrapes.keepwins.util.data.ResourceType;
import dev.eatgrapes.keepwins.util.data.ResourceUtil;
...
public class NanoFontRenderer extends MinecraftInstance {
    ...
    private NanoFontRenderer(String name, String fileName, int plainFont) {
        this.name = name + "-bold";
        NanoFontRenderer boldRendererTemp;
        try (InputStream inputStream = ResourceUtil.getResourceAsStream(fileName + "-bold.ttf", ResourceType.FONT)) {
            ...
            size = 16;
            boldRendererTemp = this; // 如果成功加载了粗体字体，则使用自身作为粗体渲染器
        } catch (NullPointerException ee) {
            Logger.debug("Bold font " + fileName + " not found.");
            font = plainFont;
            boldRendererTemp = null; // 没有找到粗体字体
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        this.boldRenderer = boldRendererTemp != null ? boldRendererTemp : this; // 如果没有粗体字体，则使用普通字体
    }
